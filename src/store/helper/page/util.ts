...
import pageStore, { IPageOptions } from '../../window';
...
export function findPageByScriptIdAndType(scriptId: number | string) {
  return pageStore.store.pages.find((p) => {
...
export async function closePageByScriptIdAndType(scriptId: number | string) {
  ...
  if (target) {
    return await pageStore.close(target.key);
  }
}
...
export function updatePageByScriptId(
  ...
  pageStore.updatePage(target.key, options, pageData);
}
